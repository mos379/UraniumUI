...
public class CollectionTreeViewNodeHolderView : VerticalStackLayout
{
    public View NodeView { get => nodeContainer.Content; set => nodeContainer.Content = value; }

    public CollectionTreeViewNodeHolderView ParentHolderView { get; private set; }

    public CollectionTreeView TreeView { get; internal set; }
    ...
    public CollectionTreeViewNodeHolderView(DataTemplate dataTemplate, CollectionTreeView treeView, BindingBase childrenBinding, int indentLevel = 0)
    {
    ...
    private void CreateChildContainer(Binding binding)
    {
        ...
        {
            var childNode = new CollectionTreeViewNodeHolderView(DataTemplate, TreeView, ChildrenBinding, this.indentLevel + 1)
            {
    ...
    protected CollectionTreeViewNodeHolderView FindHolderView(object item)
    {
        return NodeChildren.Handler?.PlatformView is not null
            ? TreeView.FindManyInChildrenHierarchy<CollectionTreeViewNodeHolderView>()
    ...
    public virtual void ApplyIsExpandedPropertyBindings()
    {
        ...
        foreach (CollectionTreeViewNodeHolderView item in Children.Where(x => x is CollectionTreeViewNodeHolderView))
        {
    ...
    public static readonly BindableProperty IsLeafProperty = BindableProperty.Create(
        nameof(IsLeaf), typeof(bool?), typeof(CollectionTreeViewNodeHolderView), null, BindingMode.TwoWay,
        propertyChanged: (bindable, oldValue, newValue) => (bindable as CollectionTreeViewNodeHolderView).OnIsLeafChanged((bool?)newValue));

    public bool IsExpanded { get => (bool)GetValue(IsExpandedProperty); set => SetValue(IsExpandedProperty, value); }

    public static readonly BindableProperty IsExpandedProperty =
        BindableProperty.Create(nameof(IsExpanded), typeof(bool), typeof(CollectionTreeViewNodeHolderView), false,
            propertyChanged: (bindable, oldValue, newValue) =>
            {
                if (bindable is CollectionTreeViewNodeHolderView holderView)
                {
    ...
    public static readonly BindableProperty IsSelectedProperty = BindableProperty.Create(
        nameof(IsSelected), typeof(bool), typeof(CollectionTreeViewNodeHolderView), false,
            propertyChanged: (bindable, oldValue, newValue) => (bindable as CollectionTreeViewNodeHolderView).IsSelectedChanged());
